...
rh_vol <- read.csv("RH_Volume.csv")
merge_rhvol <- merge(rh_vol, age, by.x = "rh.aparc.volume", by.y = "MRI.ID", all.x = FALSE, all.y = TRUE)

mergea182 <- rh_vol[substr(rh_vol$rh.aparc.volume,1,1) == "h",]
mergea182$FreesurferID <- mergea182$rh.aparc.volume
mergea182$SubID <- substr(mergea182$rh.aparc.volume,2,5)
mergea182_2 <- merge(mergea182, age, by.x = "SubID", by.y = "SubID")
mergea182_2 <- mergea182_2[,-c(2,40)]

mergeothers <- rh_vol[substr(rh_vol$rh.aparc.volume,1,1) != "h",]
mergeothers_2 <- merge(mergeothers, age, by.x = "rh.aparc.volume", by.y = "MRI.ID")
...
sink(paste0("rh_volumeresults-",model, ".txt"))
